...
`--校` 者告舊法也 `--正格` 者不許舊法也
`--整` 者因文樹整出正格源文也
文法可選 `--文法=v1` `--文法=v2` `--文法=compat`
校正之軸可分 `--校=文法` `--校=文體` `--校=字體` `--校=訓點`
返點顯示曰 `--讀順`。表記だけを正規化する時は `--整=表記` とする。

舊法如 `使甲為七` `方甲過零` `否則` `答` `而已矣` `術畢` `倍術` 亦受之

文法 v2 では値の置き場を `置七於甲` と書き、術の実引数を `用倍術以甲` と明示できる。
文字列は鉤括弧を構文にせず、従来どおり `曰辭曰嘶嗡辭畢` と書く。
